...
import { ArrowLeft, Loader2, Send, Calendar, FileText, Database, AlertCircle } from "lucide-react";
import { createReportRequest } from "@/app/actions/dashboard.action";
...
export function NewRequestForm() {
  ...
  const {
    ...
    defaultValues: {
      outputType: "file",
      fileFormat: "excel", // Default format
    ...
    try {
      ...
      if (result.error) {
        ...
        toast.success("สร้างคำขอสำเร็จ!", {
          description: "คำขอรายงานของคุณถูกส่งไปยัง Admin แล้ว",
        });
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="border-b bg-white dark:bg-gray-900">
        <div className="container mx-auto px-4 py-4">
          <Link href="/requests" className="inline-flex items-center text-muted-foreground hover:text-foreground">
      ...
      <main className="container mx-auto px-4 py-8 max-w-3xl">
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl">สร้างคำขอรายงานใหม่</CardTitle>
            <CardDescription>
              กรอกรายละเอียดรายงานที่คุณต้องการ Admin จะได้รับแจ้งเตือนทันที
            </CardDescription>
          </CardHeader>
          <CardContent>
            {/* Template Select */}
            <div className="mb-6">
              <TemplateSelect
                onSelect={(templateData) => {
                  ...
                  if (templateData.description) setValue("description", templateData.description);
                  if (templateData.outputType) setValue("outputType", templateData.outputType as any);
                  if (templateData.fileFormat) setValue("fileFormat", templateData.fileFormat as any);
                  if (templateData.dateRangeType) setValue("dateRangeType", templateData.dateRangeType as any);
                  if (templateData.priority) setValue("priority", templateData.priority as any);
                  if (templateData.sourceSystem) setValue("sourceSystem", templateData.sourceSystem as any);
                  if (templateData.dataSource) setValue("dataSource", templateData.dataSource);
                  if (templateData.additionalNotes) setValue("additionalNotes", templateData.additionalNotes);
                  toast.success("โหลด Template สำเร็จ", { description: "กรุณาตรวจสอบและแก้ไขข้อมูลตามต้องการ" });
                }}
              />
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              {/* รูปแบบไฟล์ (แสดงเฉพาะถ้าเลือก file) */}
              {outputType === "file" && (
                <div className="space-y-2">
                  ...
                  {dateRangeType === "fiscal_year" && (
                    <div className="grid md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Select onValueChange={(value) => setValue("fiscalYearStart", value)}>
                          <SelectTrigger>
                      ...
                      <div className="space-y-2">
                        ...
                        <Select onValueChange={(value) => setValue("fiscalYearEnd", value === "single" ? undefined : value)}>
                          <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  ระบุชื่อตารางหรือระบบที่ต้องการดึงข้อมูล (ถ้าทราบ)
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <RichTextEditor
                  ...
                  placeholder="ข้อมูลอื่นๆ ที่ต้องการแจ้ง Admin เช่น เงื่อนไขพิเศษ รูปแบบที่ต้องการ ฯลฯ"
                />
              </div>
